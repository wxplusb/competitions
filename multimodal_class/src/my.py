...
def read_trans(path, sort=True, add_pos=True):
    df = pd.read_csv(path,parse_dates=["transaction_dttm"],
...
class DropColDs(Dataset):
    def __init__(self, data, dftarget=None):
        if dftarget is not None:
            d = data.merge(dftarget, on="user_id").sort_values('user_id') # добавил
    ...
    def __getitem__(self, idx):
        ...
        if self.ix_del is not None:
            x = np.delete(x,self.ix_del,1)
        return x, y

def add_folds(df:pd.DataFrame, strat_col = None, group_col=None, n_folds:int=5, random_state:int=34) -> pd.DataFrame:
